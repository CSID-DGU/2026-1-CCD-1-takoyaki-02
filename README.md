...
플레이어는 실물 보드게임을 그대로 즐기고, 에이전트가 진행자(딜러)를 대신해 **인식 · 심판 · 진행 · 해설**을 수행한다.
...
게임 규칙 엔진(FSM)과 네 개의 에이전트가 협력하여 **사람 진행자 없이** 보드게임을 처음부터 끝까지 진행한다.
...
- 설계 — [FSM 설계](boardgame-ai/docs/FSM_설계.md) · [WebSocket 명세](boardgame-ai/docs/WebSocket_명세.md) · [데이터 흐름 명세](boardgame-ai/docs/데이터흐름_명세.md)
