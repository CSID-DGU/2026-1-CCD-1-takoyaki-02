...
# 등록 단계 진입(또는 플레이어 전환) 후 카드를 들어올릴 시간을 주는 유예(초).
# 이 시간 동안은 ROLE_DETECTED를 발화하지 않는다. 진입 즉시 테이블에 놓여 있던
# 카드가 곧바로 확정돼 다음 플레이어로 넘어가버리는 것을 막는다.
_ROLE_REG_GRACE_SEC = 1.5
...
class WerewolfRules:
    ...
    def __init__(self, card_tracker: CardTracker) -> None:
        ...
        # 역할 등록/최종공개 단계 진입(또는 플레이어 전환) 시각. grace 유예 계산용.
        self._reg_entry_ts: float = 0.0
    ...
    def build_candidates(
        ...
        if phase == _PHASE_ROLE_REG_TRANSITION:
            ...
            if current_reg_player and current_reg_player != self._last_reg_player:
                ...
                # 새 플레이어 진입 시각 기록 → grace 유예 시작(카드 들어올릴 텀).
                self._reg_entry_ts = perception.ts
    ...
    def _check_role_detected(
        ...
        if not actor_id or actor_id in self._reported_roles:
            return None
        # 진입 직후 유예: 카드를 들어올릴 시간을 준다. 이 시간 동안은 발화하지 않아
        # 테이블에 이미 놓여 있던 카드가 즉시 확정돼 다음 플레이어로 넘어가는 것을 막는다.
        if perception.ts - self._reg_entry_ts < _ROLE_REG_GRACE_SEC:
            return None
